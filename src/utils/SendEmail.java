...
public class SendEmail {
    ...
    // 收件人邮箱（替换为自己知道的有效邮箱）
//    public static String receiveMailAccount = "1848224883@qq.com";

    public static void main(String[] args) throws Exception {

    }
    public static void sendMessage(String receiveMailAccount,String username,String verifyCode) throws Exception {
...
//        session.setDebug(true);
        ...
        // 3. 创建一封邮件
        String html=readHtmlToString("utils/template/UserConfirmTemplate.html");
        // 写入模板内容
        Document doc = Jsoup.parse(html);
        doc.getElementById("username").html(username);
        doc.getElementById("verifyCode").html(verifyCode);
        String mailContent = doc.toString();
        //
    ...
    public static void sendFindPwdMail(String receiveMailAccount,String username,String href) throws Exception {
        // 1. 创建参数配置, 用于连接邮件服务器的参数配置
        Properties props = new Properties();                    // 参数配置
        props.setProperty("mail.transport.protocol", "smtp");   // 使用的协议（JavaMail规范要求）
        props.setProperty("mail.smtp.host", myEmailSMTPHost);   // 发件人的邮箱的 SMTP 服务器地址
        ...
        */
        ...
        // 3. 创建一封邮件
        String html=readHtmlToString("utils/template/FindPwd.html");
        // 写入模板内容
        Document doc = Jsoup.parse(html);
        doc.getElementById("username").html(username);
        doc.getElementById("ResetPwd").html(href);
        doc.getElementById("ResetPwdHref").attr("href",href);
        String mailContent = doc.toString();
        //
        MimeMessage message = createMimeMessage(session, myEmailAccount, receiveMailAccount,mailContent);
    ...
    public static String readHtmlToString(String htmlFileName) throws Exception{
        ...
        Reader reader = null;
        try {
            is = SendEmail.class.getClassLoader().getResourceAsStream(htmlFileName);
            if (is ==  null) {
